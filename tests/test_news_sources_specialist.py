"""Tests for the SPECIALIST / niche high-signal source pack.
...
from catchem.news_sources.specialist import _SPECIALIST_FEEDS, specialist_feeds


def test_provider_returns_at_least_eight_feeds() -> None:
    feeds = specialist_feeds()
    assert len(feeds) >= 8
...
def test_feed_names_are_unique_and_namespaced() -> None:
    ...
    assert all(name.startswith("spec-") for name in names)


def test_all_urls_are_https() -> None:
    for spec in specialist_feeds():
...
def test_no_name_collision_with_default_feeds() -> None:
    ...
    pack_names = {spec.name for spec in specialist_feeds()}
    assert default_names.isdisjoint(pack_names), (
        f"specialist pack reuses DEFAULT_FEEDS names: {default_names & pack_names}"
    )
...
def test_feeds_present_in_assemble_feeds() -> None:
    ...
    # At least eight spec-* feeds must survive the de-dup merge.
    spec_in_assembled = [s for s in assembled if s.name.startswith("spec-")]
    assert len(spec_in_assembled) >= 8
...
def test_source_tuple_matches_provider_output() -> None:
    ...
    # provider (the provider is a thin map over _SPECIALIST_FEEDS).
    assert len(specialist_feeds()) == len(_SPECIALIST_FEEDS)
    expected_names = {f"spec-{slug}" for slug, _url, _dom in _SPECIALIST_FEEDS}
    assert {spec.name for spec in specialist_feeds()} == expected_names
