"""Tests for the GOVERNMENT / ECONOMIC-DATA-RELEASE source pack.
...
from catchem.news_sources.govdata import _GOVDATA_FEEDS, govdata_feeds
...
def test_feed_names_are_unique_and_namespaced() -> None:
    ...
    assert all(name.startswith("gov-") for name in names)


def test_all_urls_are_https() -> None:
    for spec in govdata_feeds():
...
def test_no_name_collision_with_default_feeds() -> None:
    ...
    pack_names = {spec.name for spec in govdata_feeds()}
    assert default_names.isdisjoint(pack_names), (
        f"govdata pack reuses DEFAULT_FEEDS names: {default_names & pack_names}"
    )


def test_no_collision_with_regulators_macro_commodities_packs() -> None:
    # The govdata pack must be disjoint from the central-bank regulators pack
    # and the neighbouring macro / commodities packs. assemble_feeds() merges
    # all of them and de-dups by name (first occurrence wins), so a clash would
    # SILENTLY drop the govdata entry. Assert disjointness on the *full merged*
    # name set excluding govdata itself.
    pack_names = {spec.name for spec in govdata_feeds()}
    other_names = {
        spec.name
        for spec in assemble_feeds()
        if not spec.name.startswith("gov-")
    }
    assert other_names.isdisjoint(pack_names), (
        f"govdata pack collides with another pack / DEFAULT_FEEDS: "
        f"{other_names & pack_names}"
    )
...
def test_feeds_present_in_assemble_feeds() -> None:
    ...
    # At least eight gov-* feeds must survive the de-dup merge.
    gov_in_assembled = [s for s in assembled if s.name.startswith("gov-")]
    assert len(gov_in_assembled) >= 8
...
def test_source_tuple_matches_provider_output() -> None:
    ...
    # provider (the provider is a thin map over _GOVDATA_FEEDS).
    assert len(govdata_feeds()) == len(_GOVDATA_FEEDS)
    expected_names = {f"gov-{slug}" for slug, _url, _dom in _GOVDATA_FEEDS}
    assert {spec.name for spec in govdata_feeds()} == expected_names
