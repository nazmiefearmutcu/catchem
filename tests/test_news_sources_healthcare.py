"""Tests for the HEALTHCARE / BIOTECH / PHARMA specialist source pack.
...
from catchem.news_sources.healthcare import (
    _HEALTHCARE_FEEDS,
    healthcare_feeds,
)
...
def test_feed_names_are_unique_and_namespaced() -> None:
    ...
    assert all(name.startswith("hc-") for name in names)


def test_all_urls_are_https() -> None:
    for spec in healthcare_feeds():
...
def test_no_name_collision_with_default_feeds() -> None:
    ...
    pack_names = {spec.name for spec in healthcare_feeds()}
    assert default_names.isdisjoint(pack_names), (
        f"healthcare pack reuses DEFAULT_FEEDS names: "
...
def test_feeds_present_in_assemble_feeds() -> None:
    ...
    # At least eight hc-* feeds must survive the de-dup merge.
    hc_in_assembled = [s for s in assembled if s.name.startswith("hc-")]
    assert len(hc_in_assembled) >= 8
...
def test_source_tuple_matches_provider_output() -> None:
    ...
    # provider (the provider is a thin map over _HEALTHCARE_FEEDS).
    assert len(healthcare_feeds()) == len(_HEALTHCARE_FEEDS)
    expected_names = {f"hc-{slug}" for slug, _url, _dom in _HEALTHCARE_FEEDS}
    assert {spec.name for spec in healthcare_feeds()} == expected_names


def test_no_collision_with_any_other_pack() -> None:
    # The merged feed set (DEFAULT_FEEDS + every pack) must have no duplicate
    # names after this pack is added — assemble_feeds() de-dups by name, so a
    # silent drop of an hc-* feed (collision with another pack) would show up
    # as a global duplicate here.
